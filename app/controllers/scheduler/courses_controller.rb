class Scheduler::CoursesController < Scheduler::ApplicationController
  ...
  def index
    @courses = Course.includes(:client_group).joins('INNER JOIN course_events ON course_events.course_id = courses.id INNER JOIN admins ON admins.id = courses.teacher_id')
  ...
  def remind
    @remind = Course.remind.order(:end_date)
    @current = Course.current.where(:reminder => true).order(:end_date)
    @upcoming = Course.where(:reminder => true).where('start_date > ?', Date.today).order(:end_date)
  end
